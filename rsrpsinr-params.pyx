...
def make_coverage_params(
    ...
    cdef:
        ...
        DTYPE_t d, h_diff, tilt_s, azimuth_s 
    ...
    for si in range(num_samples):
        ...
        #for bi in range(num_bases):
            ...
            params_int.append([si, bi])
            params_float.append([pathloss, tilt_s, azimuth_s])
    #end of for-loop s
    return [np.array(params_int, dtype=np.uint32, order='C'), np.array(params_float, dtype=DTYPE, order='C')]


cdef object cget_coverage(
    np.ndarray[DTYPE_t, ndim=2] B, 
    unsigned int[:,:] params_int,
    DTYPE_t[:,:] params_float,
    int num_samples,
    double RSRP_TH=-88,
    double SINR_TH=-3,
    double NOISE=-110):
    ...
    cdef:
        unsigned int num_bases = B.shape[0]
        unsigned int num_params = params_int.shape[0]
    assert B.shape[1] == 7
    ...
    cdef:
        ...
        unsigned int si, bi
        DTYPE_t ab, tb, pb
        DTYPE_t beta, alpha, g, pathloss, v, pow_v
    
    for i in range(num_params):
        si = params_int[i, 0]
        bi = params_int[i, 1]
        pathloss = params_float[i, 0] 
        tilt_s = params_float[i, 1] 
        azimuth_s = params_float[i, 2] 
...
def get_coverage(
    ...
    double SINR_TH,
    double NOISE):
    return cget_coverage(B, params_int, params_float, num_samples,
