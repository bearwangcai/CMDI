...
if __name__ == "__main__":
    ...
    B_of_S = np.array([[i for i in range(len(S))]] * len(B))
    t2 = time.clock() - t0 - t1
    ...
    t0 = time.clock()
    pint, pfloat = rs.make_coverage_params(S,B,B_of_S,TERM_HEIGHT=1.5,FREQ=900,)
    num_samples = S.shape[0]
    t01 = time.clock()

    covered_s_count, isCovered, bIndex, rsrp, sinr, noise = rs.get_coverage(B,
        pint, pfloat,num_samples,
        RSRP_TH=-88,
        SINR_TH=-3,
        NOISE=-110
        )
    t1 = time.clock() - t01
    print(t01-t0, "seconds process time in rs.make_coverage_params")
