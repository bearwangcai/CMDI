...
cdef object cget_coverage(
    ...
    np.ndarray[DTYPE_t, ndim=2] B_of_S,
    double RSRP_TH=-88,
    double SINR_TH=-3,
    ...
    cdef:
        np.ndarray[DTYPE_t, ndim=2] Alpha = np.zeros(num_samples * num_bases, dtype=DTYPE, order='C').reshape(num_samples, num_bases)
        np.ndarray[DTYPE_t, ndim=2] Beta = np.zeros(num_samples * num_bases, dtype=DTYPE, order='C').reshape(num_samples, num_bases)
        np.ndarray[DTYPE_t, ndim=2] Pathloss = np.ones(num_samples * num_bases, dtype=DTYPE, order='C').reshape(num_samples, num_bases)*1000
        np.ndarray[DTYPE_t, ndim=2] dis = np.ones(num_samples * num_bases, dtype=DTYPE, order='C').reshape(num_samples, num_bases)
    ...
    return [Pathloss, dis, Alpha, Beta, covered_s_count] 
